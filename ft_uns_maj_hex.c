...
char	*ft_itoa_uns_maj_hex(unsigned int nb, int base)
{
	char			*str;
	unsigned int	tmp;
	int				len;
...
void	ft_pre_uns_maj_hex(t_flags *flags, int *space, int *zero, int *len)
{
...
void	ft_next_uns_maj_hex(t_flags *flags, char *nbr, int *len)
{
	int		space;
	int		zero;
	int		i;
	...
	i = 0;
	ft_pre_uns_maj_hex(flags, &space, &zero, len);
...
void	ft_uns_maj_hex(va_list ap, t_flags *flags)
{
	...
	nbr = ft_itoa_uns_maj_hex(va_arg(ap, unsigned int), 16);
	len = ft_strlen(nbr);
	if (nbr && flags->dot && !flags->precision && nbr[0] == '0')
		len = 0;
	ft_next_uns_maj_hex(flags, nbr, &len);
	free(nbr);
}
